...
abstract class Parser : ProductParsable {
    ...
    companion object {
        fun create(productURL: String): Parser {
            if(productURL.contains("dns-shop", false)){
                return DNSShopParser()
            }
            if(productURL.contains("mvideo", false)){
                return MvideoStoreParser()
            }
            if(productURL.contains("eldorado", false)){
                return EldoradoParser()
            }
            throw Exception("Не найден парсер")
        }
    ...
    public fun parseProduct(url: String, onParseProductListener: OnParseProductListener) {
